-- =============================================================================
-- Init SQL: run ONCE against the first Patroni leader.
-- Invocation (Patroni container must be healthy, otherwise connection refused):
--   docker compose exec -T patroni psql -U postgres -h 127.0.0.1 -f - < scripts/init-databases.sql
-- =============================================================================

-- pdns user: replace CHANGE_ME_pdns with the real password BEFORE running, OR run
-- ./scripts/sync-pdns-postgres-password.sh after init — otherwise the role password
-- will differ from PDNS_DB_PASSWORD and connections via HAProxy will fail.
DO $$ BEGIN
   CREATE ROLE pdns LOGIN PASSWORD 'CHANGE_ME_pdns';
...
SELECT 'CREATE DATABASE powerdnsadmin OWNER pdns'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'powerdnsadmin')\gexec

-- PowerDNS schema (official schema from PowerDNS docs, gpgsql backend, v4.9)
\c pdns
